...
# 2018 PbPb
INPUTMCPbPb="/mnt/T2_US_MIT/submit-hi2/scratch/gwangjun/crab_Bfinder_20190624_Hydjet_Pythia8_Official_BuToJpsiK_1033p1_pt3tkpt0p7dls2_allpthat_pthatweight.root"
#INPUTMCPbPb="/mnt/T2_US_MIT/submit-hi2/scratch/gwangjun/crab_Bfinder_20190520_Hydjet_Pythia8_BuToJpsiK_1033p1_pt3tkpt0p7dls2_v2_addSamplePthat_pthatweight.root" #private MC
INPUTDATAPbPb="/mnt/T2_US_MIT/submit-hi2/scratch/gwangjun/crab_Bfinder_20190513_HIDoubleMuon__PsiPeri__HIRun2018A_04Apr2019_v1_1033p1_GoldenJSON_skimhltBsize_ntKp.root"
...
#SETTING for NP fit
INPUTMCPbPb_NP="/mnt/T2_US_MIT/submit-hi2/scratch/gwangjun/NP/crab_Bfinder_20190520_Hydjet_Pythia8_BuToJpsiK_nonprompt_1033p1_pt3tkpt0p7dls2_allpthat_pthatweight.root"
...
g++ fitOnSavedB.C $(root-config --cflags --libs) -g -o fitOnSavedB.exe 
e./fitOnSavedB.exe 1 "$OUTPUTFILEPbPbSAVEHIST"  "$INPUTMCPbPb"  "$TRGPbPb" "$CUTPbPb"   "$SELGENPbPb"   "$ISMCPbPb"   1   "$ISDOWEIGHTPbPb"   "$LABELPbPb"  "$OUTPUTFILEPbPb" "$NPFIT_PbPb" 0 "$CENTPbPbMIN" "$CENTPbPbMAX"
rm fitOnSavedB.exe
fi 
...
./fitBY.exe 0 "$INPUTDATAPP"  "$INPUTMCPP"  "$TRGPP" "$CUTPP"  "$SELGENPP"   "$ISMCPP"   1   "$ISDOWEIGHTPP"   "$LABELPP"  "$OUTPUTFILEPP_Y" "$NPFIT_PP" 0
rm fitBY.exe
fi
...
./MCefficiencyY.exe  0 "$INPUTMCPP"  "$SELGENPP" "$SELGENPPACCPP&&"  "$RECOONLYPP" "$CUTPP&&$TRGPPMC"  "$LABELPP" "$OUTPUTFILEMCSTUDYPP_Y" "$ISDOWEIGHTPP" "$CENTPbPbMIN" "$CENTPbPbMAX"
...
#g++ fitB.C $(root-config --cflags --libs) -g -o fitB.exe 
#./fitB.exe 0 "$INPUTMCPP"  "$INPUTMCPP"  "$TRGPPMCClosure" "$CUTPP"   "$SELGENPP"   "$ISMCPPMCClosure"   "$LUMIPPMCClosure"   "$ISDOWEIGHTPPMCCosure"   "$LABELPPMCClosure"  "$OUTPUTFILEPPMCClosure" "$NPFIT_PP" 0
#rm fitB.exe
