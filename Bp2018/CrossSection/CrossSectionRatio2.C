...
void CrossSectionRatio(TString inputFONLL="", TString input="", TString efficiency="",TString outputplot="",int usePbPb=1,TString label="PbPb",int doDataCor = 0,double lumi=1.,Float_t centMin=0.,Float_t centMax=90.)
{
	...
	TFile* fileRealRef = new TFile("ppref_extrapolation.root");
	TGraphAsymmErrors* realref_d = (TGraphAsymmErrors*)fileRealRef->Get("real_ref_new_data");
	TGraphAsymmErrors* realref_f = (TGraphAsymmErrors*)fileRealRef->Get("real_ref_new_FONLL");
	...
	realref_f->SetFillColor(kOrange);
	realref_d->SetLineColor(kRed);
	//realref_f->Draw("A2 same");
	//realref_d->Draw("ep same");
