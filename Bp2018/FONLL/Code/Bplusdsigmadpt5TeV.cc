...
double rebin[REBINp] = {5,10,15,30,50};


void Bplusdsigmadpt5TeV(TString input="pp",double norm=0.405)
{
  ...
  std::ifstream getdata(Form("../FONLLInputs/%s_FONLL_xsection.txt",input.Data()));

  if(!getdata.is_open())
    {
      std::cout<<"Opening the file fails"<<std::endl;
    }
